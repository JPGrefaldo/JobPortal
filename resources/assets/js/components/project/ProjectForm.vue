<template>
    <div class="card mb-8">
        ...
        <div class="md:flex py-3">
            <div class="md:w-1/3 pr-6">
                <span class="block md:text-right mt-4 font-header text-blue-dark font-semibold mb-3"
                    >Project type</span
                >
                <small class="block md:text-right text-red" v-show="errors.has('Project Type')">{{
        ...
        <div class="md:flex py-3">
            <div class="md:w-1/3 pr-6">
                ...
                <small
                    ...
                    v-show="errors.has('Project Information')"
                    >{{ errors.first('Project Information') }}</small
                >
            </div>
            <div class="md:w-2/3">
                <textarea
        ...
        <div class="md:flex py-3">
            <div class="md:w-1/3 pr-6">
                <span class="block md:text-right mt-4 font-header text-blue-dark font-semibold mb-3"
                    >Project city/area</span
                >
                <small
                    class="block md:text-right text-red"
                    v-show="errors.has('Project Location')"
                    >{{ errors.first('Project Location') }}</small
                >
            </div>
            <div class="md:w-2/3">
                <input
        ...
        <div class="py-6">
            <div class="md:flex">
                <div class="md:w-1/3 pr-8">
                    <span class="font-bold font-header text-blue-dark block md:text-right mb-3"
                        >Post add on these websites:</span
                    >
                    <small
                        class="block md:text-right text-red"
                        v-show="errors.has('Post add on these websites')"
                        >{{ errors.first('Post add on these websites') }}</small
                    >
                </div>
    ...
    methods: {
        ...
        submitProject() {
            ...
            if (this.errors.all().length == 0) {
                let endpoint =
                    this.mode == 'edit' ? 'project/updateProject' : 'project/saveProject';

                if (this.project.remotes.length != 1 && this.project.remotes[0] != 'all') {
                    this.project.remotes = this.getRemoteIds(
                        this.sites.filter(site => site.checked)
                    );
                }

                this.$store.dispatch(endpoint, this.project).then(response => {
