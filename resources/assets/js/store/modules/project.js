...
export const actions = {
    ...
    fetchByTypes(context) {
        axios.get('/api/producer/project/type').then(response => {
            context.commit(types.PROJECT_TYPES, response.data.projectType);
        });
    },

    fetchProjectJobs(context) {
        return axios.get('/api/producer/project/jobs').then(response => {
    ...
    saveProjectJob(context, job) {
        return axios.post('/api/producer/project/jobs', job).then(response => {
    ...
    updateProjectJob(context, job) {
        return axios.put(`/api/producer/project/jobs/${job.id}`, job).then(response => {
            context.commit(types.JOBS, response.data.job);
        });
    },
